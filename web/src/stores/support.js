...
import { ref } from 'vue'
import { getVersion, donateSupport } from '@/api/system'
...
// - 【已支持】上报成功后本版本内不再提示。
// 「本次运行」由后端 /api/version 返回的 started_at 区分：刷新页面不重置，
// 应用重启后 started_at 变化，弹窗会再次出现。
//
...
//
// 赞赏横幅与悬浮按钮常驻展示（关闭不持久化）：刷新浏览器后横幅再次出现，
// 直到用户支持过当前版本才隐藏；悬浮按钮始终可以打开赞赏弹窗。
//
// 赞赏完全自愿：不赞赏不影响任何功能。
...
function isSupportedFor(appVersion, storedVersion) {
...
export const useSupportStore = defineStore('support', () => {
  ...
  async function init(isAdmin, info) {
    if (!isAdmin) return
    if (info) {
      appVersion.value = info.version || ''
      serverStartedAt.value = info.started_at || ''
    } else {
      try {
        const { data } = await getVersion()
        appVersion.value = data?.version || ''
        serverStartedAt.value = data?.started_at || ''
      } catch {
        /* 版本接口失败不影响使用 */
      }
    }
  ...
  function scheduleModal() {
    ...
    // 弹窗重新出现；未拿到 started_at 时按「本次运行」降级处理。
   ...
   *  启动弹窗会再次出现，再次确认即刷新记录。 */
  async function confirmSupported(amount) {
    sending.value = true
    errorText.value = ''
    let ok = false
    try {
      const { data } = await donateSupport(amount)
      ok = data?.ok === true
    } catch {
      ok = false
    }
